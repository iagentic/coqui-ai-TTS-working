...
def tts2():
    with lock:
        ...
        logger.info("Speaker idx: %s", speaker_idx)
        logger.info("Language idx: %s", language_idx)
        # wavs = api.tts(text, speaker=speaker_idx, language=language_idx, style_wav=style_wav, speaker_wav="/home/ubuntu/coqui-ai-TTS/voices/small.mp3")
        print("Text is ::",text)
        wavs = api.tts(text=text, speaker="Kumar Dahl", language="en", style_wav=style_wav, speaker_wav="/home/ubuntu/coqui-ai-TTS/voices/small.mp3")
        out = io.BytesIO()
        api.synthesizer.save_wav(wavs, out)
    return send_file(out, mimetype="audio/wav")
